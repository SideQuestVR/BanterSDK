...
*/
    ...
    public class BanterRigidbody : BanterComponentBase
    {
        ...
        [Method] public void _AddForce(Vector3 force, ForceMode mode){
            _rigidbody.AddForce(force, mode);
        }
        [Method] public void _MovePosition(Vector3 position){
            _rigidbody.MovePosition(position);
        }
        [Method] public void _MoveRotation(Quaternion rotation){
            _rigidbody.MoveRotation(rotation);
        }
        [Method(overload = "Values")] public void _AddForce(float x, float y, float z, ForceMode mode) {
            _rigidbody.AddForce(x, y, z, mode);
        }
        [Method] public void _Sleep() {
            _rigidbody.Sleep();
        }
        [Method] public void _AddExplosionForce(float explosionForce, Vector3 explosionPosition, float explosionRadius, float upwardsModifier, ForceMode mode){
