...
namespace BS.SDKEditor
{
    ...
    [CustomEditor(typeof(BSGeometry), true)]
    public class BSGeometryEditor : Editor
    {
        void OnEnable()
        {
            if (target is BSGeometry)
            {
                var script = (BSGeometry)target;
