...
namespace BS.SDKEditor
{
    ...
    [CustomEditor(typeof(BSPhysicsMaterial), true)]
    public class BSPhysicsMaterialEditor : Editor
    {
        void OnEnable()
        {
            if (target is BSPhysicsMaterial)
            {
                var script = (BSPhysicsMaterial)target;
