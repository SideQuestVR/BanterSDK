...
namespace Banter{
    [CustomEditor(typeof(BanterMaterial))]
    public class BanterMaterialEditor : Editor{
        void OnEnable() {
            if(target is BanterMaterial){
                var script = (BanterMaterial)target;
...
var seeFields = new Label("shaderName, texture, color, side, generateMipMaps, ");
